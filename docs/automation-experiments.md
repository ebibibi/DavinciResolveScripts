...
- まずはText+/Fusionタイトルで「SECTION」「章タイトル」を表示する。
...
- `text_title` は通常テロップ、章タイトル、KeyPoint、SECTIONカードに利用する。
...
最初はText+だけONにし、SEとアイキャッチは実機検証しながら段階的に有効化する。

## 優先順位

1. **Text+自動テロップの安定化**
   - 現在の章/KeyPoint Text+挿入を土台に、通常字幕・キーワード補助・章カードへ拡張する。
2. **SE挿入の最小実験**

...
Whisperセグメントを15〜75秒程度の連続した話題区間へまとめ、Claude CLIが全区間を一度だけ要約する。頻出語ではなく「視聴者がいま何を理解できるか」を表す具体的な一文を作り、画面左上の小さなText+として、次の話題が始まるまで途切れさせない。Claude CLIがない場合や生成に失敗した場合は、説明・方法・仕組み・違いなどを含む発話を優先するローカル要約へ戻る。
...
- `InsertTitleIntoTimeline()` / `InsertFusionTitleIntoTimeline()` はトラック番号を直接指定できず、V1ロックも実機では無視された。メインタイムラインでは使用せず、同梱プロジェクトのMedia PoolにあるネイティブText+ `テロップ` を `AppendToTimeline()` の `trackIndex: 2` でV2へ配置する。
- V2追加後は `GetTrackTypeAndIndex()` で実配置先を検証し、V2以外なら非リップル削除する。TextPlusの `Style=Regular` と `Font=HGPSoeiKakugothicUB` を両方読み戻し、完全一致を確認する。
...
Whisper解析結果を直接タイムライン操作に変換せず、いったん動画別の `_ai_assist/{動画名_サイズ_mtime}/ai_edit_plan.json` に編集アクションとして保存する。
...
- 2026-07-16の実機確認で、V1ロック方式でもText+がV1へ入り本編を押し出すことを確認。複合クリップ経由でV2に逃がす方式も、Text+そのものではなくなるため廃止。同梱テンプレートのネイティブText+素材をV2へ直接Appendする。

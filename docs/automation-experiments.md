...
- `InsertTitleIntoTimeline()` / `InsertFusionTitleIntoTimeline()` はトラック番号を直接指定できず、V1ロックも実機では無視された。メインタイムラインでは使用せず、一時タイムラインでText+を複合クリップ化してから `AppendToTimeline()` の `trackIndex: 2` でV2へ配置する。
- V2追加後は `GetTrackTypeAndIndex()` で実配置先を検証し、V2以外なら非リップル削除する。フォントもTextPlusの `Font` を読み戻し、`HGPSoeiKakugothicUB` との完全一致を確認する。
...
- 2026-07-16の実機確認で、V1ロック方式でもText+がV1へ入り本編を押し出すことを確認。メインタイムラインへの直接タイトル挿入を廃止し、一時タイムライン＋複合クリップ＋V2明示Appendへ変更した。

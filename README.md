...
- AI編集補助（試験機能）
  - `whisper` CLIが利用できる場合、録画音声を文字起こしして章マーカーとキーポイントText+/Fusionタイトル草案をタイムラインに追加
  - AIが選んだ冒頭フック文を、DaVinci Resolve上で直接編集できるText+/FusionタイトルとしてV2に追加
...
- DaVinci Resolveのタイムラインに章マーカーとキーポイントText+/Fusionタイトルを追加
  - Blue marker: 章
  - Yellow: キーポイントタイトル
  - Text+/FusionタイトルはV2に追加し、フォントは `HGPSoeiKakugothicUB` を指定
  - QC確認ポイントは最終映像に出さず、`_ai_assist/ai_assist_status.txt` と `ai_edit_plan.json` に残します
- AIが選んだ冒頭フック文をText+/Fusionタイトルとして本編先頭のV2に追加。mp4などの焼き込み動画ではないため、Resolve上で文言・見た目・尺を直接編集できます
...
`ai_edit_actions` は実験的な自動編集機能のON/OFFです。既定ではまず安定化を優先し、KeyPointのText+/FusionタイトルだけをV2へ追加します。`captions` は通常テロップです。`section_cards`、`section_break_videos`、`sound_effects` は `experimental_full_actions=true` を明示した場合だけ動く高度な実験機能です。`chapter_titles` は互換用の設定名ですが、チャプターはText+ではなくタイムラインマーカーとして追加します。
...
- **AI編集補助**: whisper文字起こしが可能な環境では、章マーカー・キーポイント・冒頭フックを自動生成。Text+/FusionタイトルはV2に追加

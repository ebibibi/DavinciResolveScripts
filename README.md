...
- AI編集補助（試験機能）
  - `whisper` CLIが利用できる場合、録画音声を文字起こしして章・キーポイントのText+/Fusionタイトル草案をタイムラインに追加
  - AIが選んだ冒頭フック文を、DaVinci Resolve上で直接編集できるText+/Fusionタイトルとして本編先頭に追加
...
- DaVinci Resolveのタイムラインに章とキーポイントをText+/Fusionタイトルとして追加
  - Blue: 章タイトル
  - Yellow: キーポイントタイトル
  - QC確認ポイントは最終映像に出さず、`_ai_assist/ai_assist_status.txt` と `ai_edit_plan.json` に残します
- AIが選んだ冒頭フック文をText+/Fusionタイトルとして本編先頭に追加。mp4などの焼き込み動画ではないため、Resolve上で文言・見た目・尺を直接編集できます
- `whisper` が見つからない場合や文字起こしに失敗した場合は、タイムライン上の追加は行わず、`ai_assist_status.txt` に理由を書き出します

Text+/Fusionタイトルは、そのまま最終映像に載っても成立するたたき台です。必要に応じてDaVinci Resolve上で削除・調整してください。
...
`ai_edit_actions` は実験的な自動編集機能のON/OFFです。`captions`、`chapter_titles`、`key_point_titles`、`section_cards` はText+/Fusionタイトルを追加し、`section_break_videos` は区切り動画、`sound_effects` はSEを追加します。
...
- **AI編集補助**: whisper文字起こしが可能な環境では、章・キーポイント・冒頭フックを編集可能なText+/Fusionタイトルとして自動生成

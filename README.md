...
- AI編集補助（試験機能）
  - 完成済みのauto-editor無音カットを高品質な中間動画へレンダリング
  - カット済み動画をWhisperで再文字起こしし、AIがインパクトのある発言だけを選定
  - FFmpeg/ASSで強調テロップを焼き込み、完成候補をResolveへ自動配置
  - FCPXML、テロップなし中間動画、ASS、選定JSONを残して再編集経路を維持
...
- 音声区間が1つも検出されず `Timeline is empty` になった場合は、無音カットなしで録画全体を残すResolve XMLを自動生成して処理を続行
...
### 6. AI編集補助（有償版・試験機能）
- auto-editorのResolve XML出力は従来どおり最初に実行し、無音自動カットを必ず維持
- 同じ `audio:threshold=1%` / `--margin 0.5sec` で、高ビットレートのカット済み中間動画も生成
- カット済み中間動画を `whisper` で再文字起こしするため、時刻は完成本編と直接一致
- Claude CLIが強調すべき発言を最大12件選び、利用できない場合はローカル規則へフォールバック
- `HGPSoeiKakugothicUB` を使った横動画用ASSを生成し、FFmpegでテロップを焼き込み
- 焼き込み済みの単一クリップとエンディングをmainタイムラインへ配置
...
- 章マーカーは引き続きResolveタイムラインへ追加
- `whisper`、Claude CLI、FFmpeg、完成候補のImportMediaのいずれかが失敗した場合は、従来の自動カットXMLタイムラインへ自動で戻る

Resolve APIによるText+/Fusion自動配置は既定経路から外しました。テロップを直す場合は `ai_highlights.json` または `ai_highlights.ass` を編集して再レンダリングします。大きく編集し直す場合は、従来どおり生成されるFCPXMLをResolveへ読み込めます。
...
```json
{
  ...
  "post_cut_ai_titles": {
    "enabled": true,
    "cut_master_video_bitrate": "40M",
    "cut_master_audio_bitrate": "320k",
    "final_crf": 16,
    "final_preset": "medium",
    "max_titles": 12,
    "min_gap_seconds": 10,
  ...
  "ai_edit_actions": {
    ...
    "viewer_focused_topic_labels": true,
...
`post_cut_ai_titles.enabled=true` が新しい既定経路です。auto-editorの無音カットを単一動画へ書き出し、そのカット後時間軸でAIテロップを生成します。`cut_master_video_bitrate` は1回目の高品質中間、`final_crf` はテロップ焼き込み版の品質です。ビットレートは大きいほど高品質・大容量、CRFは小さいほど高品質・大容量です。
`ai_edit_actions` は旧Resolve Text+方式との互換設定です。`post_cut_ai_titles.enabled=true` で完成候補が生成された場合、Text+自動配置は実行しません。外部レンダリングが失敗して従来XMLへ戻った場合のみ、これらの設定が使われます。`chapter_markers` はどちらの経路でも有効です。
...
- **AI編集補助**: auto-editorのカット済み動画を再文字起こしし、Claude CLI＋ローカルフォールバックで強調発言を選定。FFmpeg/ASSで焼き込み、Resolveへ単一クリップとして配置

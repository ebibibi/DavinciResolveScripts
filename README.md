...
`ai_edit_actions` は実験的な自動編集機能のON/OFFです。既定ではまず安定化を優先し、KeyPointのText+/FusionタイトルだけをV2へ追加します。`captions` は通常テロップです。`chapter_markers` はチャプターのタイムラインマーカー追加を制御します。`section_cards`、`section_break_videos`、`sound_effects` は `experimental_full_actions=true` を明示した場合だけ動く高度な実験機能です。`chapter_titles` は互換用の設定名ですが、チャプターはText+ではなくタイムラインマーカーとして追加します。
